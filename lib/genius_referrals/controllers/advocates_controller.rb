...
module GeniusReferrals
  class AdvocatesController < BaseController
    ...
    def get_share_links(account_slug, 
      ...
      # prepare query url
      _query_builder = Configuration.base_uri.dup
      _query_builder << '/accounts/{account_slug}/advocates/{advocate_token}/share-links'
      _query_builder = APIHelper.append_url_with_template_parameters _query_builder, {
        'account_slug' => account_slug,
        'advocate_token' => advocate_token
      }
    ...
    # @param [String] account_slug Required parameter: The advocate's token
    # @param [String] advocate_token Required parameter: The advocate's token
    # @param [Integer] advocate_payment_method_id Required parameter: The payment method's identifier
    # @param [PaymentMethodForm] advocate_payment_method_form Required parameter: The body of the request
    # @return void response from the API call
    def put_payment_method(account_slug, 
                           advocate_token, 
                           advocate_payment_method_id, 
                           advocate_payment_method_form)

      # prepare query url
      _query_builder = Configuration.base_uri.dup
      _query_builder << '/accounts/{account_slug}/advocates/{advocate_token}/payment-methods/{advocate_payment_method_id}'
      _query_builder = APIHelper.append_url_with_template_parameters _query_builder, {
        'account_slug' => account_slug,
        'advocate_token' => advocate_token,
        'advocate_payment_method_id' => advocate_payment_method_id
      }
      _query_url = APIHelper.clean_url _query_builder
      ...
      # prepare and execute HttpRequest
      _request = @http_client.put _query_url, headers: _headers, parameters: advocate_payment_method_form.to_json
      CustomHeaderAuth.apply(_request)
      _context = execute_request(_request)
      validate_response(_context)
    end
    ...
    def post_payment_method(account_slug, 
      ...
      # prepare query url
      _query_builder = Configuration.base_uri.dup
      _query_builder << '/accounts/{account_slug}/advocates/{advocate_token}/payment-methods'
      _query_builder = APIHelper.append_url_with_template_parameters _query_builder, {
        'account_slug' => account_slug,
        'advocate_token' => advocate_token
      }
      ...
      # prepare and execute HttpRequest
      _request = @http_client.post _query_url, headers: _headers, parameters: advocate_payment_method_form.to_json
      CustomHeaderAuth.apply(_request)
      _context = execute_request(_request)
      validate_response(_context)
    ...
    # @param [String] bonuses_redemption_method_slug Required parameter: The bonus redemption method's identifier
    # @return Mixed response from the API call
    def get_bonus_redemption_method(bonuses_redemption_method_slug)

      # prepare query url
      _query_builder = Configuration.base_uri.dup
      _query_builder << '/utilities/bonuses-redemption-methods/{bonuses_redemption_method_slug}'
      _query_builder = APIHelper.append_url_with_template_parameters _query_builder, {
    ...
    # @return Mixed response from the API call
    def get_bonus_redemption_methods
    ...
    def get_currencies
      ...
      # prepare query url
      _query_builder = Configuration.base_uri.dup
      _query_builder << '/utilities/currencies'
      _query_url = APIHelper.clean_url _query_builder
    ...
    def get_currency(code)
      ...
      # prepare query url
      _query_builder = Configuration.base_uri.dup
      _query_builder << '/utilities/currencies/{code}'
      _query_builder = APIHelper.append_url_with_template_parameters _query_builder, {
        'code' => code
      }
      _query_url = APIHelper.clean_url _query_builder
    ...
    # @param [Integer] page Optional parameter: Page number, e.g. 1 would start at the first result, and 10 would start at the tenth result.
    # @param [Integer] limit Optional parameter: Maximum number of results to return in the response. Default (10), threshold (100)
    # @param [String] filter Optional parameter: Allowed fields: username, is_active. Use the following delimiters to build your filters params. The vertical bar ('\|') to separate individual filter phrases and a double colon ('::') to separate the names and values. The delimiter of the double colon (':') separates the property name from the comparison value, enabling the comparison value to contain spaces. Example: www.example.com/users?filter='name::todd\|city::denver\|title::grand poobah'
    # @param [String] sort Optional parameter: Allowed fields: username, created. Use sort query-string parameter that contains a delimited set of property names. For each property name, sort in ascending order, and for each property prefixed with a dash ('-') sort in descending order. Separate each property name with a vertical bar ('\|'), which is consistent with the separation of the name\|value pairs in filtering, above. For example, if we want to retrieve users in order of their last name (ascending), first name (ascending) and hire date (descending), the request might look like this www.example.com/users?sort=last_name\|first_name\|-hire_date
    # @return Mixed response from the API call
